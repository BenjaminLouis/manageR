...
header <- switch(tolower(params$info$doc),
                 ...
                 invoice = "Delivery :",
                 quote = "Client :")
box1 <- paste0("<span class = \"box\"><p class = \"headclient\">", header, 
               "</p>",paste(lclient, collapse = ""), "</span>")
if (tolower(params$info$doc) %in% c("facture", "invoice")) {
  ...
  header <- switch(tolower(params$info$doc),
                   facture = "Facturation :",
                   invoice = "Billing :")
  box2 <- paste0("<span class = \"box\"><p class = \"headclient\">", header, 
                 "</p>",paste(lbilling, collapse = ""), "</span>")
} else {
  box2 <- NULL
}
HTML(paste(box1, box2, sep = ""))
```
