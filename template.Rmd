...
data <- tibble('Désignation' = character(),
                   ...
                   'Total' = double())
#if (!(params$services %in% c("NA", "")) & !is.na(params$services)) {
  #temp <- params$services
...
data %>%
  kable(format = "html") %>%
  column_spec(column = 1, width = "42%") %>%
  column_spec(column = 2, width = "13%") %>%
  column_spec(column = 3, width = "12%") %>%
  column_spec(column = 4, width = "18%") %>%
  column_spec(column = 5, width = "15%") %>%
  HTML()
```
